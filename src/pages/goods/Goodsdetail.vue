<template>
  <div class="goodsdetail">
    ...
    <slider :imgList="goodsData.album"/>
    <goodstitle :product="productSpes" :brief="goodsData.brief" :buyCount="goodsData.buy_count" :unit="goodsData.unit"/>
    <goodsservice :promotion="checkPromotion"/>
      ...
      <yd-tab-panel class="params" label="产品参数">
        <ul v-if="params.length">
          <li v-for="(item, index) in params" :key="index">
            <div v-if="item.value"><span id="paramsleft">{{ item.name }} : </span><span id="paramsright">{{ item.value }}</span>
            </div>
          </li>
        </ul>
        <ul v-else>没有详细参数</ul>
