  ...
  <div class="goodslist">
    <van-dropdown-menu>
      ...
      <van-dropdown-item title="筛选" ref="item">
        <van-switch-cell v-model="switch1" title="自营"/>
        <van-switch-cell v-model="switch2" title="库存"/>
    ...
    <yd-infinitescroll :callback="loadMore" :distance="2" :scroll-top="false" ref="infinitescrollDemo"
      ...
      <yd-list theme="4" slot="list">
        <yd-list-item v-for='(item, key) in list' :key='key' type="link"
                      :href="{path:'goodsdetail',query:{goods_id:item.id}}">
          <img slot='img' :src='item.image_url'>
          <span slot='title'>{{item.name}}</span>
          <yd-list-other slot='other'>
            <div>
              <span class='demo-list-price'><em>¥</em>{{item.price}}</span>
              <span class='demo-list-point-price'><em>¥</em>{{'100.00'}}</span>
              <span class='demo-list-self-sell'>自营</span>
              <span class='demo-list-sold-number'>已售：99</span>
            </div>
          </yd-list-other>
        </yd-list-item>
      </yd-list>
...
<style>
  ...
  .goodslist .demo-list-point-price {
    margin-left: .2rem;
    margin-right: .2rem;
  }

  .goodslist .demo-list-self-sell {
    margin-left: .2rem;
    margin-right: .2rem;
  }
</style>
