<template>
  <div class="firmorder">
    <openstore v-if="isOpenStore === 1 && storeInfo" :storeTab="storeTab"
            :store="storeInfo" :ship="shipInfo" @storeTab="selectStoreTab" @consignee="consignee"
            @shipHandler="shipHandler" @storeHandler="storeHandler"></openstore>
    <orderadd v-else :ship="shipInfo" @shipHandler="shipHandler"></orderadd>
          ...
          <yd-tab-panel label="已领取优惠券">
            <yd-cell-group class="pop-cell">
              <div v-if="userCoupon.length">
                <yd-cell-item @click.native="couponChecked(index, item.checked)" v-for="(item, index) in userCoupon"
                              :key="index">
                  <span slot="left">{{ item.name }}</span>
                  <span slot="right" class="demo-list-price"><img :src="couponCheck[0]" v-show="!item.checked"/><img
                          :src="couponCheck[1]" v-show="item.checked"/></span>
                </yd-cell-item>
              </div>
              <div v-else>
                没有优惠券 快去领取一个吧
