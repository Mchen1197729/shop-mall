    ...
    <yd-tab v-model="storeTab" :prevent-default="false" :item-click="storeClick">
      <yd-tab-panel label="快递配送">
        ...
        <div class="orderadd" v-else>
          <yd-button size="small" type="danger" style="position: relative;left: 50%;transform: translateX(-50%);"
                     @click.native="newShipAdd">新增收货地址
          </yd-button>
        </div>
      </yd-tab-panel>
    ...
    <yd-popup v-model="openWindow" position="bottom" width="20%" height="60%">
      <div class="orderadd-content">
        <div v-if="shipList.length">
          <div style="margin-top: 20px; text-align: left;" v-for="(item, index) in shipList" :key="index"
               @click="shipHandler(index)">
            <div class="orderadd-top">
    ...
    <yd-popup v-model="openStore" position="bottom" width="20%" height="60%">
      <div class="orderadd-content">
        ...
        <div v-if="storeList.length">
          <div style="margin-top: 20px; text-align: left;" v-for="(item, index) in storeList" :key="index"
               @click="storeHandler(index)">
            <div class="orderadd-top">
